...
<div class="player">
  ...
  <span style="font-size:12px;opacity:.9">Te la Dedico  <b>cancion.mp3</b>Detalles Mr<b>💙🌹♥️</b> (Reproductor Mr”)</span>
...
playBtn.onclick = async () => {
  try { if (audio.paused) { await audio.play(); playBtn.textContent='⏸︎'; } else { audio.pause(); playBtn.textContent='▶︎'; } }
...
pickFile.onchange = () => {
  if (pickFile.files && pickFile.files[0]) {
    ...
    audio.src = url; audio.play().then(()=> playBtn.textContent='⏸︎').catch(()=>{});
  }
};
...
} catch(e) { showError('WebGL parece desactivado. Prueba con Chrome/Edge/Firefox, o habilita aceleración por hardware.'); return; }
...
const phrases = ["Pinchecha ✨", " Hermosa 💛", "Mi niña", "Mi amor", "Hermosota ✨", "Mi Infinita ♾️", "Mi Todo", "Me Encantas 🥰", " galaxia💫", "Te Amo💗", "Mi Canción Favorita 🎶", "Unica 😊", "Mi Reina 👑", "Mi Amor", "Mi Mundo", "Mi Luz 🌠", "Mi Niña", "Mi Tesoro💛", "Mi Paz", "Mi Eterna", "Mi Sueño", "Mi Pensamiento Favorito", "Mi Bombom", "Mi Diosa", "Te Amo Infinitamente ♾️"];
